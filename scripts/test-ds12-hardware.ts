...
/**
 * DS12 Hardware Testing Script (TypeScript)
 * 
 * PURPOSE:
 ...
 * 
 * USAGE:
 * npx ts-node scripts/test-ds12-hardware.ts [port] [address]
 * 
 * EXAMPLES:
 * npx ts-node scripts/test-ds12-hardware.ts /dev/ttyUSB0 0x00
 * npx ts-node scripts/test-ds12-hardware.ts COM3 0x00
 * 
 * SAFETY FEATURES:
...
class DS12HardwareTester {
  private controller: DS12Controller;
  private port: string;
  private address: number;
  private startTime: number;

  constructor(port: string, address: number = DEFAULT_ADDRESS) {
    this.port = port;
   ...
   * Test 2: Hardware Communication
   */
  async testHardwareCommunication(): Promise<void> {
    console.log('\n📡 Test 2: Hardware Communication');
    console.log('-'.repeat(40));
    ...
    try {
      ...
      if (Array.isArray(statusResult)) {
        ...
        // Display slot states
        statusResult.forEach((slot: any, index: number) => {
  ...
  generateSummaryReport(): void {
    ...
    const connectionSuccess = testResults.connectionTests.some(test => 
      test.tests.some((t: any) => t.name === 'connection-establishment' && t.success)
    );
    const communicationSuccess = testResults.statusTests.some(test =>
      test.tests.some((t: any) => t.name === 'status-check' && t.success)
    );
    ...
    if (testResults.errorLog.length > 0) {
      console.log('\n❌ Errors Encountered:');
      testResults.errorLog.forEach((error: any, index: number) => {
...
async function main(): Promise<void> {
  ...
  console.log('🚀 DS12 Hardware Testing Script (TypeScript)');
