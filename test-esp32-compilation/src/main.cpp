...
bool initializeCU12() {
  Serial.println("MEDICAL: Initializing CU12 Hardware Communication...");
  ...
  Serial.printf("MEDICAL: RS485 DE pin configured on GPIO %d\n", RS485_DE_PIN);
  ...
  Serial.printf("MEDICAL: Built-in RS485 initialized - RX: GPIO%d, TX: GPIO%d, Baud: %d\n", 
                RS485_RX_PIN, RS485_TX_PIN, CU12_BAUD_RATE);
  
  // Test CU12 communication with version request
  for (int attempt = 1; attempt <= CU12_MAX_RETRIES; attempt++) {
    Serial.printf("MEDICAL: CU12 Connection attempt %d/%d\n", attempt, CU12_MAX_RETRIES);
    ...
    if (sendCU12Command(CU12_GET_VERSION)) {
      ...
      if (readCU12Response(response, sizeof(response))) {
        if (response[4] == CU12_ASK_SUCCESS) {
          cu12Available = true;
          cu12Mode = "live";
          Serial.println("MEDICAL: ✅ CU12 Hardware validated and operational");
          return true;
        }
      }
    }
