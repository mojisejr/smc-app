...
export async function checkStateCommand(options: CheckStateOptions = {}): Promise<void> {
...
function displaySimpleSlotTable(slotStates: SlotState[]): void {
  ...
  slotStates.forEach(slot => {
    ...
    const statusColor = slot.isOpen ? chalk.red(statusText) : chalk.green(statusText);
