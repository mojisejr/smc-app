...
program
  ...
  .option(
    "--type <type>",
    "License type: production, internal, or development (default: production)",
    "production"
  )
  .option(
    "--test-mode",
...
Examples (Phase 9 - WiFi-Free):
  $ smc-license generate -o "SMC Medical" -c "HOSP001" -a "SMC_Cabinet" -e "2025-12-31"
  $ smc-license generate -o "SMC Internal" -c "INT001" -a "SMC_Test" -e "2025-06-30" --type internal
  $ smc-license generate -o "Test Org" -c "TEST001" -a "SMC_Test" -e "2025-06-30" --test-mode --type development
...
`
  ...
  .action(async (options: any) => {
    try {
      ...
      const validTypes = ["production", "internal", "development"];
      ...
      // Internal license validation - require organization to be "SMC" or contain "SMC"
      if (options.type === "internal") {
        if (!options.org.toLowerCase().includes("smc")) {
          throw new Error(
            'Internal licenses can only be generated for SMC organization (organization must contain "SMC")'
          );
        }
        console.log(
          chalk.cyan(
            `🔒 Generating ${options.type} license for internal SMC use`
          )
        );
      } else {
        console.log(chalk.cyan(`📄 Generating ${options.type} license`));
      }
      ...
      if (options.wifiSsid || options.wifiPassword) {
        console.log(
          chalk.yellow(
            "⚠️  Phase 9: WiFi credentials are deprecated and will be ignored"
          )
        );
        console.log(
          chalk.gray(
            "   WiFi connection will be handled manually during deployment"
          )
        );
      }

      console.log(
        chalk.blue("🚀 Phase 9: WiFi-free license generation starting...")
      );

      if (options.testMode) {
        console.log(
          chalk.yellow(
            "🧪 Test Mode: Generating license with mock MAC address (WiFi-free)"
          )
        );
...
program
  .command("batch")
  .description(
    "Process multiple licenses from CSV file generated by ESP32 Deployment Tool"
  )
  .requiredOption(
    "--input <csvFile>",
    "Input CSV file from ESP32 Deployment Tool"
  )
  .option(
    "--output-dir <dir>",
    "Output directory for license files (default: same as CSV location)"
  )
  .option("--update-csv", "Update CSV file with processing status", false)
  .option(
    "--skip-existing",
    "Skip records where license file already exists",
    false
  )
  .option(
    "--expiry-days <days>",
    "Set expiry X days from now (overrides CSV expiry)",
    (val) => parseInt(val)
  )
  .option(
    "--expiry-months <months>",
    "Set expiry X months from now (overrides CSV expiry)",
    (val) => parseInt(val)
  )
  .option(
    "--expiry-years <years>",
    "Set expiry X years from now (overrides CSV expiry)",
    (val) => parseInt(val)
  )
  .option("--no-expiry", "Set license to never expire (overrides CSV expiry)")
  .option(
    "--dry-run",
    "Validate CSV and show what would be processed without generating licenses"
  )
  ...
  .action(async (options: any) => {
    try {
      ...
      if (options.preview) {
        ...
        console.log(
          chalk.green(`Completed records: ${preview.stats.completed}`)
        );
        ...
        if (preview.sampleRecords.length > 0) {
          ...
          preview.sampleRecords.forEach((record, index) => {
            console.log(
              chalk.white(
                `${index + 1}. ${record.customerId} - ${record.organization}`
              )
            );
            console.log(
              chalk.gray(
                `   App: ${record.applicationName}, Expiry: ${record.expiryDate}`
              )
            );
          });
        }
      ...
      const batchOptions: BatchOptions = {
        ...
        outputDir: options.outputDir
          ? require("path").resolve(options.outputDir)
          : undefined,
        updateCSV: options.updateCsv,
        skipExisting: options.skipExisting,
        expiryDays: options.expiryDays,
      ...
      if (result.failed > 0) {
        ...
        if (result.skipped > 0) {
          console.log(
            chalk.yellow(`Skipped ${result.skipped} existing licenses`)
          );
        }
        if (options.dryRun) {
          console.log(
            chalk.blue("This was a dry run - no files were generated")
          );
        }
      }
    } catch (error: any) {
      ...
      if (error.message.includes("not found")) {
        ...
        console.log(
          chalk.gray("2. Check CSV was generated by ESP32 Deployment Tool")
        );
        console.log(
          chalk.gray("3. Verify CSV header format matches expected format")
        );
        ...
        console.log(
          chalk.gray(
            "2. Use valid values for days (1-3650), months (1-120), years (1-10)"
          )
        );
        console.log(
          chalk.gray("3. Check that CSV expiry dates are in YYYY-MM-DD format")
        );
      } else {
        console.log(
          chalk.gray("1. Try using --dry-run to validate CSV format")
        );
        console.log(
          chalk.gray("2. Use --verbose flag for detailed error information")
        );
        console.log(
          chalk.gray(
            "3. Check file permissions for input and output directories"
          )
        );
        console.log(
          chalk.gray("4. Use --preview to analyze CSV without processing")
        );
      }
  ...
  .action(async (options: any) => {
    try {
      ...
      if (validationResult.isValid && validationResult.fileInfo) {
        ...
        console.log(
          chalk.white(`Data Length:      ${info.encrypted_data_length} chars`)
        );
        console.log(chalk.white(`KDF Algorithm:    ${info.kdf_algorithm}`));
        console.log(
          chalk.white(
            `Has KDF Context:  ${info.has_kdf_context ? "Yes" : "No"}`
          )
        );

        console.log(chalk.blue("\n🔐 HKDF v2.0 License Information:"));
        console.log(
          chalk.green("• Self-contained license with dynamic key derivation")
        );
        ...
        console.log(
          chalk.gray(
            "• License content is encrypted and cannot be displayed without MAC address"
          )
        );
        console.log(
          chalk.gray(
            "• Use 'smc-license info' with MAC/WiFi parameters for full details (future feature)"
          )
        );
        console.log(
          chalk.gray("• This validation only checks file format and structure")
        );
        ...
        console.log(chalk.yellow("\n🔧 Common Issues:"));
        console.log(
          chalk.gray("• Ensure this is a valid HKDF v2.0 license file")
        );
        console.log(chalk.gray("• Check that the file is not corrupted"));
        console.log(
          chalk.gray(
            "• Verify the file was generated with SMC License CLI v2.0+"
          )
        );
...
`
  ...
  .action(async (options: any) => {
    try {
      ...
      console.log(
        chalk.white(
          `   Encrypted Data Length: ${info.encrypted_data_length} characters`
        )
      );
      console.log(chalk.white(`   KDF Algorithm: ${info.kdf_algorithm}`));
      ...
      console.log(
        chalk.white(
          `   Salt (Base64): ${licenseFile.kdf_context.salt.substring(
            0,
            20
          )}...`
        )
      ...
      console.log(
        chalk.white(
          `   Info Context: ${licenseFile.kdf_context.info.substring(0, 50)}...`
        )
      );
      console.log(
        chalk.white(`   KDF Algorithm: ${licenseFile.kdf_context.algorithm}`)
      );

      // แสดงข้อมูล security features
      console.log(chalk.blue("\n🛡️  Security Features:"));
      console.log(
        chalk.green(`• Self-contained license with HKDF key derivation`)
      );
      ...
      console.log(
        chalk.green(`• Deterministic key generation from license content`)
      );
      ...
      console.log(chalk.blue("\n📝 Usage Information:"));
      console.log(
        chalk.white("• This license is compatible with SMC License CLI v2.0+")
      );
      console.log(
        chalk.white(
          "• License content can only be decrypted by the target SMC application"
        )
      );
      console.log(
        chalk.white(
          "• Hardware binding ensures license works only on designated ESP32 device"
        )
      );
      console.log(
        chalk.white(
          "• No additional setup or shared key files required for deployment"
        )
      );
...
program
  ...
  .description(
    "Update license expiry date without regenerating license (payment control)"
  )
  .requiredOption(
    "-f, --file <filename>",
    "Path to existing license file (.lic format)"
  )
  .requiredOption(
    "-e, --new-expiry <date>",
    'New expiry date in YYYY-MM-DD format (e.g., "2026-12-31")'
  )
  .requiredOption(
    "--mac-address <mac>",
    "ESP32 MAC address for validation (XX:XX:XX:XX:XX:XX)"
  )
  .requiredOption("--wifi-ssid <ssid>", "WiFi SSID for validation")
  .option(
    "--output <filename>",
    "Output filename (default: overwrites original)"
  )
  .addHelpText(
    "after",
    `
  ...
  .action(async (options: any) => {
    try {
      ...
      if (
        ...
        throw new Error(
          "License must be HKDF format (version 2.0.x or 2.1.x). Please regenerate license with current CLI version."
        );
      }
      ...
      if (licenseData.macAddress !== options.macAddress.toUpperCase()) {
        throw new Error(
          `MAC address mismatch. License is bound to: ${licenseData.macAddress}`
        );
      }

      if (licenseData.wifiSsid !== options.wifiSsid) {
        throw new Error(
          `WiFi SSID mismatch. License is bound to: ${licenseData.wifiSsid}`
        );
      }
      ...
      updatedLicenseData.checksum = require("crypto")
        .createHash("sha256")
        .update(checksumData)
        .digest("hex")
        .slice(0, 16);
      ...
      console.log(chalk.gray("====================================="));
      console.log(
        chalk.white(`Organization:     ${updatedLicenseData.organization}`)
      );
      console.log(
        chalk.white(`Customer ID:      ${updatedLicenseData.customerId}`)
      );
      console.log(
        chalk.white(`Application ID:   ${updatedLicenseData.applicationId}`)
      );
      console.log(
        chalk.white(`MAC Address:      ${updatedLicenseData.macAddress}`)
      );
      console.log(chalk.white(`Previous Expiry:  ${licenseData.expiryDate}`));
      console.log(
        chalk.yellow(`New Expiry:       ${updatedLicenseData.expiryDate}`)
      );
      console.log(
        chalk.white(`Updated At:       ${updatedLicenseData.generatedAt}`)
      );

      // Calculate new validity period
      const daysUntilExpiry = Math.ceil(
        (newExpiryDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)
      );
      console.log(chalk.green(`Valid for:        ${daysUntilExpiry} days`));

      console.log(chalk.blue("\n🎆 Payment Control Benefits:"));
      console.log(
        chalk.green("• License extended without hardware re-binding")
      );
      console.log(chalk.green("• Same application binary remains valid"));
      console.log(
        chalk.green("• Customer can continue using existing installation")
      );
      ...
      if (error.message.includes("MAC address")) {
        console.log(
          chalk.gray(
            "1. Verify MAC address matches the original license binding"
          )
        );
        console.log(chalk.gray("2. Use uppercase format: AA:BB:CC:DD:EE:FF"));
        console.log(
          chalk.gray("3. Check MAC address from ESP32 device directly")
        );
      } else if (error.message.includes("WiFi")) {
        console.log(
          chalk.gray("1. Verify WiFi SSID matches the original license")
        );
        ...
        error.message.includes("HKDF") ||
        error.message.includes("version")
      ) {
        console.log(
          chalk.gray(
            "1. License must be generated with CLI v2.0+ (HKDF format)"
          )
        );
        console.log(chalk.gray("2. Regenerate license if using old format"));
        console.log(
          chalk.gray("3. Old dynamic key licenses are not compatible")
        );
      } else if (error.message.includes("date")) {
        console.log(chalk.gray("1. Use YYYY-MM-DD format for expiry date"));
        ...
        console.log(
          chalk.gray("2. Check that all required parameters are provided")
        );
...
`
  ...
  .action(async () => {
    try {
      ...
      console.log(
        chalk.white(
          "Current system uses HKDF (Key Derivation Function) technology:"
        )
      );
      console.log("");
      ...
      console.log(
        chalk.gray("  • Single key for all licenses (security risk)")
      );
      ...
      console.log(
        chalk.gray("• Old v1.0 licenses still work with legacy parsers")
      );
      console.log(
        chalk.gray("• New v2.0 HKDF licenses provide enhanced security")
      );
      console.log(
        chalk.gray("• Regenerate licenses for maximum security benefits")
      );
      console.log("");

      console.log(
        chalk.green(
          "🎉 HKDF system provides maximum security without complexity!"
        )
      );
    } catch (error: any) {
...
program.configureHelp({
  ...
  subcommandTerm: (cmd: any) => cmd.name(),
});
...
program
  ...
  .option(
    "--update-expiry <customer-id> <new-date>",
    "Update expiry date in registry"
  )
    ...
    try {
      ...
      if (options.add) {
        ...
        if (!options.customerId || !options.organization) {
          console.log(
            chalk.red(
              "❌ --customer-id and --organization are required for add operation"
            )
          );
          process.exit(1);
        }
      ...
      if (options.updateExpiry) {
        ...
        console.log(
          chalk.blue(`🔄 Updating expiry for ${customerId} to ${newDate}...`)
        );

        await registry.updateExpiryInRegistry(
          customerId,
          newDate,
          options.notes
        );
      ...
      if (options.export !== undefined) {
        ...
        console.log(
          chalk.white(
            `   File location: ${registry.getRegistryFilePath(targetDate)}`
          )
        );
        return;
      }
      ...
      console.log(chalk.yellow("\n🔧 Troubleshooting:"));
      console.log(
        chalk.gray("1. Ensure registry directory exists and is writable")
      );
