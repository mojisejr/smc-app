...
export class LicenseRegistry {
  ...
  async ensureRegistryDir(): Promise<void> {
    try {
      ...
      if (this.verbose) {
        console.log(chalk.cyan(`info: Created registry directory: ${this.registryDir}`));
      }
    }
  }
   ...
   */
  getCSVHeader(): string {
    return 'timestamp,customer_id,organization,application_name,expiry_date,license_status,license_file,mac_address,wifi_ssid,created_at,updated_at,notes';
  }
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (fields.length >= 12) {
          entries.push({
            ...
            license_file: fields[6],
            mac_address: fields[7],
            wifi_ssid: fields[8],
            created_at: fields[9],
            updated_at: fields[10],
            notes: fields.slice(11).join(',') // รวม notes ที่อาจมี comma
          });
        }
      }
   ...
   */
  async addLicenseEntry(licenseFilePath: string, options: {
    customerData?: {
      customer_id: string;
      organization: string;
      application_name: string;
    ...
    if (existing) {
      ...
      if (this.verbose) {
        console.log(chalk.yellow(`info: Updated existing entry for ${entry.customer_id}`));
      }
    } else {
      // เพิ่มใหม่
      entries.push(entry);
      ...
      if (this.verbose) {
        console.log(chalk.green(`info: Added new registry entry for ${entry.customer_id}`));
      }
    }
   ...
   */
  async updateExpiryInRegistry(customerId: string, newExpiryDate: string, notes?: string): Promise<void> {
    const entries = await this.readOrCreateRegistry();
    ...
    if (this.verbose) {
      console.log(chalk.green(`info: Updated expiry for ${customerId} to ${newExpiryDate}`));
    }
  }
  ...
  async getRegistryStats(date?: Date): Promise<{
    ...
    entries.forEach(entry => {
      byStatus[entry.license_status] = (byStatus[entry.license_status] || 0) + 1;
    });
    ...
    if (stats.total > 0) {
      ...
      Object.entries(stats.byStatus).forEach(([status, count]) => {
        const color = status === 'active' ? chalk.green : 
                     status === 'expired' ? chalk.red :
                     status === 'updated' ? chalk.yellow : chalk.white;
      ...
      recentEntries.forEach(entry => {
        console.log(chalk.white(`   ${entry.customer_id} (${entry.organization}) - ${entry.license_status}`));
      });
    }
