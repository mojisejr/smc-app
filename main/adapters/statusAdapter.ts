...
export const registerUniversalCheckLockedBackHandler = (
  ...
  ipcMain.handle("check-locked-back", async (event, payload) => {
    try {
      ...
      console.log(
        `[UNIVERSAL-ADAPTER] check-locked-back routing to ${hardwareInfo.type} for slot ${payload.slotId}`
      );
      ...
      if (hardwareInfo.type === "DS12" && cu12StateManager) {
        ...
        try {
          ...
          const isLocked = await cu12StateManager.checkSlotLockStatus(
            payload.slotId
          );
        ...
        details: {},
          ...
          if (isLocked) {
            ...
            if (slotData) {
              const isUnlockOperation =
                slotData.dataValues.opening && !slotData.dataValues.occupied;
              const isDispenseOperation =
                slotData.dataValues.opening && slotData.dataValues.occupied;
              ...
              if (isUnlockOperation) {
                ...
                console.log(
                  `[CU12-STATUS] Unlock completed for slot ${payload.slotId} - frontend sync triggered`
                );
                ...
                console.log(
                  `[CU12-STATUS] Dispense completed for slot ${payload.slotId} - showing Clear/Continue modal`
                );
              }
            }
          } else {
            ...
            if (slotData) {
              ...
              if (isUnlockOperation) {
                ...
                console.log(
                  `[CU12-STATUS] Slot ${payload.slotId} still open - keeping unlock modal open`
                );
              } else if (isDispenseOperation) {
                ...
                console.log(
                  `[CU12-STATUS] Slot ${payload.slotId} still open - keeping dispense modal open`
                );
              }
            }
          }
          ...
          return {
            ...
            isLocked: isLocked,
            message: isLocked
              ? "Slot is closed - operation complete"
              : "Slot is still open - please close slot",
            userControlled: true,
          };
        } catch (error) {
          console.error(
            `[CU12-STATUS] User-controlled status check failed:`,
            error.message
          );
          throw error;
        }
