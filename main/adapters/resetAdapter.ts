...
export const registerUniversalResetHandler = (
  ...
  ipcMain.handle('reset', async (event, payload) => {
    try {
      ...
      console.log(`[UNIVERSAL-ADAPTER] reset routing to ${hardwareInfo.type} for slot ${payload.slotId}`);
      ...
      if (hardwareInfo.type === 'CU12' && cu12StateManager) {
        ...
        await cu12StateManager.enterOperationMode(`reset_slot_${payload.slotId}`);
...
export const registerUniversalForceResetHandler = (
  ...
  ipcMain.handle('force-reset', async (event, payload) => {
    try {
      ...
      console.log(`[UNIVERSAL-ADAPTER] force-reset routing to ${hardwareInfo.type} for slot ${payload.slotId}`);
      
      await logger({
        user: 'system',
        message: `Universal force-reset request: slot ${payload.slotId}, HN: ${payload.hn}, hardware: ${hardwareInfo.type}`,
      });

      if (hardwareInfo.type === 'CU12' && cu12StateManager) {
        // Route to CU12 force reset operation
        console.log(`[CU12-FORCE-RESET] Processing force reset for slot ${payload.slotId}`);
        
        await cu12StateManager.enterOperationMode(`force_reset_slot_${payload.slotId}`);
        
        try {
          // CU12 force reset implementation - unlock regardless of status
          const unlockSuccess = await cu12StateManager.performUnlockOperation(payload.slotId);
          
          await logger({
            user: 'system',
            message: `CU12 force reset completed: slot ${payload.slotId}, HN: ${payload.hn}, success: ${unlockSuccess}`,
          });

          // Send force reset success event (same as KU16)
          mainWindow.webContents.send("force-reset-success", {
            slotId: payload.slotId,
            hn: payload.hn,
            message: 'บังคับรีเซ็ตสำเร็จ'
          });
        ...
        // Route to KU16 force reset operation
        console.log(`[KU16-FORCE-RESET] Processing force reset for slot ${payload.slotId}`);
        
        if (!ku16Instance.connected) {
          await logger({
            user: 'system',
            message: `KU16 force reset failed: connection error for slot ${payload.slotId}`,
          });
          
          throw new Error('KU16 connection error');
        }

        // Use existing KU16 force reset logic
        const result = await ku16Instance.forceReset(payload);
        
        await logger({
          user: 'system',
          message: `KU16 force reset completed: slot ${payload.slotId}, HN: ${payload.hn}, success: ${result.success}`,
        });

        return result;

      } else {
        const errorMsg = `Hardware ${hardwareInfo.type} not initialized or not supported for force reset operation`;
        console.error(`[UNIVERSAL-ADAPTER] ${errorMsg}`);
        
        await logger({
          user: 'system',
          message: `Universal force reset error: ${errorMsg}`,
        });
      ...
      await logger({
        user: 'system',
        message: `Universal force reset error: slot ${payload.slotId}, error: ${error.message}`,
      });
      ...
      mainWindow.webContents.send('force-reset-error', {
        slotId: payload.slotId,
        hn: payload.hn,
        message: 'เกิดข้อผิดพลาดในการบังคับรีเซ็ต',
        error: error.message
      });
