  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isLockError && attempt < maxRetries) {
        ...
        logDebug(
          "background",
          `Database lock detected in ${operationName}, retrying in ${delay}ms (attempt ${attempt}/${maxRetries})`
        );
...
(async () => {
  ...
  try {
    ...
    logSystemInfo(
      "background",
      "Electron app ready, proceeding with initialization"
    );
    ...
    try {
      ...
      if (!isProd) {
        ...
        logDebug(
          "background",
          "Development mode: Database schema updated with alter"
        );
      } else {
        sql = await executeWithRetry(
          () => sequelize.sync(),
    ...
    const indicator = new IndicatorDevice({}, mainWindow);
    ...
    mainWindow.webContents.once("did-finish-load", () => {
      logSystemInfo(
        "background",
        "Renderer finished loading, navigating to initial page",
        {
          initialPage,
        }
      );
    ...
    logSystemInfo(
      "background",
      "SMC application initialization completed successfully",
      {
...
app.on("window-all-closed", async () => {
  ...
  try {
    logSystemInfo(
      "background",
      "Application shutdown initiated - cleaning up controllers"
    );
