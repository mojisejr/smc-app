  ...
  ipcMain.handle('cu12-dispense', async (event, payload) => {
    ...
    try {
      ...
      if (!user) {
        await logger({
          user: 'system',
          message: `CU12 dispense: user not found`,
        });
      ...
      if (!targetSlot || !targetSlot.isLocked) {
        throw new Error(`Slot ${payload.slotId} is not in locked state for dispensing`);
      }
      ...
      await logger({
        user: 'system',
        message: `CU12 dispense: slot #${payload.slotId} by ${userName}`,
      });
      ...
      await logger({
        user: 'system',
        message: `CU12 dispense: slot #${payload.slotId} by ${userName} error - ${error.message}`,
      });
