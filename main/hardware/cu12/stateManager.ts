...
export class CU12SmartStateManager {
  ...
  constructor(
    ...
    config?: Partial<CU12MonitoringConfig>
  ) {
  ...
  async initialize(deviceConfig: CU12Config): Promise<boolean> {
    try {
      await this.structuredLogger.info(
        "Initializing CU12 Smart State Manager",
        {
      ...
      await this.structuredLogger.info(
        "CU12 Smart State Manager initialized successfully",
        {
  ...
  async syncSlotStatus(
    strategy: "ON_DEMAND" | "HEALTH_CHECK" | "PRE_OPERATION" | "POST_OPERATION"
  ): Promise<SlotStatus[]> {
    ...
    try {
      ...
      if (
        ...
        strategy !== "PRE_OPERATION"
      ) {
        const cached = this.resourceOptimizer.getCachedResponse<SlotStatus[]>(
          "slot_status",
          this.config.cacheTimeout
        );
        if (cached) {
          await this.structuredLogger.trace("Slot status served from cache", {
            strategy,
            cached: true,
          });
          return cached;
      ...
      // Send init-res event to frontend for real-time updates (compatibility with KU16)
      await this.sendInitResEvent(deviceStatus);
  ...
  private detectStateChanges(
    deviceStatus: SlotStatus[],
    dbSlots: any[]
  ): StateChange[] {
    const changes: StateChange[] = [];

    // NOTE: Disabled auto-sync for isActive field as it's an admin setting, not hardware state
    // isActive should only be changed through admin operations, not hardware sync

    // Future: Consider syncing other fields like connection status, but not isActive
    console.log(
      "[CU12-STATE] detectStateChanges: Admin settings (isActive) are not auto-synced from hardware"
    );

    return changes; // Return empty for now - admin settings managed separately
  }
  ...
  private createStructuredLogger(): CU12StructuredLogger {
    ...
    return {
      ...
      async error(message: string, metadata?: Record<string, any>) {
        if (
          ["TRACE", "DEBUG", "INFO", "WARN", "ERROR"].includes(config.logLevel)
        ) {
