...
export class SystemWiFiManager {
  ...
  static async scanNetworks(): Promise<WiFiNetwork[]> {
    ...
    try {
      ...
      await logHardwareOperation(
        `WiFi scan completed - found ${networks.length} networks`,
        {
  ...
  static async connectToNetwork(
    ...
    boolean | { requiresManualConnection: boolean; platform: string }
  > {
    const timer = new PerformanceTimer();
    ...
    try {
      ...
      // Enhanced bypass detection with multiple validation layers
      ...
      if (shouldBypass) {
        ...
        await logHardwareOperation(
          "WiFi connection bypassed due to enhanced bypass logic",
          {
            operation: "wifi_connect_enhanced_bypass",
            ssid: ssid,
            validation_mode: validationMode,
            build_type: buildType,
            esp32_bypass: esp32Bypass,
            internal_build_mode: internalBuildMode,
      ...
      if (wifiStrategy === "manual") {
        console.log(
          "info: [MANUAL] Manual WiFi connection required - showing instructions to user"
        );
        await logger({
          user: "system",
      ...
      console.log(
        `info: [AUTO] Attempting automatic WiFi connection on ${this.platform}`
      );

      await logHardwareOperation(
        `Starting automatic WiFi connection on ${this.platform}`,
        {
      ...
      for (let retryCount = 1; retryCount <= 3; retryCount++) {
        console.log(
          `info: Checking WiFi connection (attempt ${retryCount}/3)...`
        );

        await logHardwareOperation(
          `WiFi connection check attempt ${retryCount}`,
          {
        ...
        if (retryCount < 3) {
          console.log(
            "info: Connection check failed, retrying in 2 seconds..."
          );
      ...
      if (isConnected) {
        ...
        console.error(
          `error: Failed to connect to WiFi: ${ssid} (verified with ${3} attempts)`
        );

        await logHardwareOperation(
          `WiFi connection failed after verification attempts`,
          {
  ...
  static async isConnectedTo(ssid: string): Promise<boolean> {
    try {
      ...
      if (validationMode === "bypass") {
        console.log(
          `info: [BYPASS] Skipping WiFi connection check for: ${ssid}`
        );
      ...
      console.log(
        `debug: Current network: ${currentNetwork}, Target: ${ssid}, Connected: ${isConnected}`
      );
      return isConnected;
    } catch (error) {
   ...
   */
  private static async createWindowsWiFiProfile(
    ssid: string,
    password: string
  ): Promise<void> {
    try {
      ...
      switch (this.platform) {
        ...
        case "darwin":
          ...
          if (
            ...
            console.log(
              `debug: Found network using pattern 3: ${trimmedOutput}`
            );
